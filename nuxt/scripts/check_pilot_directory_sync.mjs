...
const clientHeartbeatSource = read('app/composables/useClientHeartbeat.ts')
const runtimeReportingSource = read('app/services/monitoring/clientRuntimeReportingService.ts')
...
assert.equal(clientHeartbeatSource.includes('coachId'), false, 'Client heartbeat must not write coachId')
assert.equal(runtimeReportingSource.includes('coachId'), false, 'Runtime reporter must not write coachId')
assert.equal(runtimeReportingSource.includes('role:'), false, 'Runtime reporter must not write role')
