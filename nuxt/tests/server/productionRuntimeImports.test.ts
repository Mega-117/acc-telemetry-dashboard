...
describe('production runtime imports', () => {
  ...
  it('preserves dynamic routes across the GitHub Pages fallback', () => {
    ...
    expect(appShell).toContain('const queryRedirectPath = pendingSpaRedirectPath.value')
    expect(appShell).toContain("const destination = pendingSpaRedirectPath.value || (isDashboardRoute(normalizedRoutePath.value) ? route.fullPath : '/panoramica')")
