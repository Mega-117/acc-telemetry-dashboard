...
describe('target voice renderer/IPC wiring contract', () => {
  ...
  it('persists voice directly from the main list, independently of target edits', () => {
    ...
    expect(panel).toContain('@click="setTargetLapVoiceEnabled(!targetLapVoiceEnabled)"')
    expect(panel).toContain('data-overlay-wheel-action="target-voice"')
    expect(panel).toContain(':aria-pressed="targetLapVoiceEnabled"')
