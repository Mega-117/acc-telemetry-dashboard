...
describe('driver swap without reloading the room', () => {
  it('updates room presence while preserving the unavailable strategy recipient', async () => {
    fakes.roomDoc = room({ membershipModel: 'social', memberUids: ['me', 'popo'] })
    const a = member({ protocolVersion: 3, connected: true })
    const b = member({ uid: 'popo', nickname: 'popo', protocolVersion: 3, connected: true, driving: false })
    const link = await open([a, b])
    expect(link.selectedTargetUid.value).toBe('me')
    fakes.pushMembers?.([{ ...a, driving: false }, { ...b, driving: true }])
    expect(link.crew.value.filter(row => row.driving).map(row => row.uid)).toEqual(['popo'])
    expect(link.roomDriving.value.executor?.uid).toBe('popo')
    expect(link.selectedTargetUid.value).toBe('me')
    expect(link.canSend.value).toBe(false)
    expect(link.carSnapshot.value).toBeNull()
    expect(await link.sendPlan({ fuelLiters: 20 })).toBe(false)
    await vi.advanceTimersByTimeAsync(90_000)
    expect(link.crew.value.find(row => row.uid === 'popo')?.driving).toBe(true)
    expect(fakes.calls).not.toContain('send')
    link.selectTarget('popo')
    expect(link.executor.value.executor?.uid).toBe('popo')
    expect(link.canSend.value).toBe(true)
    fakes.pushMembers?.([{ ...a, driving: false }, { ...b, driving: true, connected: false }])
    expect(link.crew.value.some(row => row.driving)).toBe(false)
    expect(link.canSend.value).toBe(false)
  })

  it('shows both active drivers independently of the selected recipient', async () => {
    fakes.roomDoc = room({ membershipModel: 'social', memberUids: ['me', 'popo'] })
    const link = await open([member()])
    fakes.pushMembers?.([member(), member({ uid: 'popo', nickname: 'popo' })])
    expect(link.crew.value.filter(row => row.driving).map(row => row.uid).sort()).toEqual(['me', 'popo'])
    expect(link.roomDriving.value.reason).toBe('multiple-driving')
    expect(link.roomDriving.value.executor).toBeNull()
    expect(link.executor.value.executor?.uid).toBe('me')
  })
})
