...
describe('App protected runtime route contract', () => {
  ...
  it('avvia e spegne gli ascolti globali (feed, Pit Wall) insieme alla dashboard di un utente che puo entrare', () => {
    ...
    expect(appSource).toContain('[appState, currentUser, canEnterApp, pitwallDemand]')
    expect(appSource).toContain('pitwallStore.start(needsPitwall)')
    expect(appSource).toContain("normalizedRoutePath.value === '/pitwall'")
    expect(appSource).toContain('|| !!pitwallStore.myRoom.value || !!pitwallStore.pitwall.value.roomId')
