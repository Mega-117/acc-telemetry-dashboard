...
describe('Pitwall cutover: legacy Firestore protocol is closed for every client', () => {
  const paths = ['pitwallGrants/a_b', 'pitwallRooms/room', 'pitwallRooms/room/members/driver',
    'pitwallRooms/room/control/activeOrder', 'pitwallRooms/room/orders/order',
    'pitwallVehicles/fingerprint', 'pitwallSessions/driver', 'pitwallSessions/driver/orders/order',
    'pitwallSessions/driver/signals/signal']
  it('denies reads, creates, updates and deletes even to former participants and admins', async () => {
    await testEnv.withSecurityRulesDisabled(async context => {
      for (const path of paths) await setDoc(doc(context.firestore(), path), {
        driverUid: PILOT_UID, engineerUid: COACH_UID, status: 'pending', hostUid: PILOT_UID,
        memberUids: [PILOT_UID, COACH_UID], managerUids: [PILOT_UID], allowedUids: [COACH_UID],
      })
    })
    for (const uid of [PILOT_UID, COACH_UID, ADMIN_UID]) {
      const db = testEnv.authenticatedContext(uid).firestore()
      for (const path of paths) {
        await assertFails(getDoc(doc(db, path)))
        await assertFails(updateDoc(doc(db, path), { status: 'applied' }))
        await assertFails(deleteDoc(doc(db, path)))
        await assertFails(setDoc(doc(db, `${path}-new`), { driverUid: uid, status: 'pending' }))
      }
    }
  })
})
