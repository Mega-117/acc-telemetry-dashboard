...
export function boundPitwallStrategy(strategy: unknown, nowIso: string): PitwallStrategySnapshot | null {
  ...
  return {
    ...
    ...(Array.isArray(source.applicationMethods) ? { applicationMethods: ['standard', 'mfd-v4'].filter(method => (source.applicationMethods as unknown[]).includes(method)) } : {}),
...
// insieme al resto della Race Room. Qui restava un secondo posto dove
// rispondere alla stessa domanda con dati diversi - il battito ISO scritto dal
// client invece del timestamp del server - ed e esattamente il modo in cui due
// verita divergono senza che nessuno se ne accorga (Principio 2).
