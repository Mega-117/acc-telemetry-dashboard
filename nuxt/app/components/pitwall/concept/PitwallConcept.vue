...
const searchRequest = ref<null | "race" | "permanent">(null);
const recentRequestMenu = ref<string | null>(null);
const recentPending = ref(["paolo"]);
...
const searchResult = computed(() =>
  submittedSearch.value
    ? findPitwallConceptPerson(submittedSearch.value)
    : null,
);
...
function requestSearchAccess(type: "race" | "permanent") {
  searchRequest.value = type;
}

function requestRecentAccess(id: string) {
  recentPending.value = [...new Set([...recentPending.value, id])];
  recentRequestMenu.value = null;
}
...
<template> <section class="pwc" data-testid="pitwall-concept"> <template v-if="screen === 'home'"> <div class="pwc-home"> <main class="pwc-home__main"> <section class="pwc-find-panel"> <header> <span class="pwc-eyebrow">Directory piloti</span> <h2>Trova una persona</h2> <p>Cerca un nickname. Il risultato resta separato dalle tue Crew e dai collegamenti recenti.</p> </header> <form class="pwc-find-form" @submit.prevent="submitSearch"> <label class="pwc-search"> <svg viewBox="0 0 24 24" aria-hidden="true"> <circle cx="11" cy="11" r="7"/> <path d="m20 20-4-4"/> </svg> <input v-model="search" placeholder="Nome o nickname" aria-label="Nome o nickname" /> <button v-if="search" type="button" aria-label="Cancella ricerca" @click="clearSearch">×</button> </label> <button class="pwc-btn is-primary" type="submit" :disabled="!search.trim()">Cerca</button> </form> <div v-if="submittedSearch" class="pwc-search-outcome" aria-live="polite"> <article v-if="searchResult" class="pwc-search-person"> <span class="pwc-avatar" :class="`is-${searchResult.source}`">{{ searchResult.initials }}</span> <div> <strong>{{ searchResult.handle }}</strong> <small>{{ searchResult.name }}</small> <span>{{ searchResult.detail }}</span> </div> <div v-if="searchRequest" class="pwc-request-sent"> Richiesta {{ searchRequest === "race" ? "per questa gara" : "permanente" }} inviata </div> <div v-else-if="searchResult.access === 'pending'" class="pwc-request-sent"> Richiesta in attesa </div> <button v-else-if="searchResult.state === 'racing' && searchResult.access !== 'none'" class="pwc-btn is-primary" @click="go('live')" > Assisti </button> <div v-else class="pwc-access-actions"> <button v-if="searchResult.state === 'racing'" class="pwc-btn" @click="requestSearchAccess('race')" > Accesso questa gara </button> <button class="pwc-btn is-outline" @click="requestSearchAccess('permanent')"> Accesso permanente </button> </div> </article> <div v-else class="pwc-search-empty"> <span>?</span> <div> <strong>Nessun utente trovato</strong> <small>Controlla il nickname e riprova.</small> </div> </div> </div> <div v-else class="pwc-find-idle"> <span>01</span> <p>Scrivi il nickname esatto o una parte del nome, poi premi Cerca.</p> </div> </section> </main> <aside class="pwc-home-sidebar"> <section class="pwc-crews"> <header> <div> <span class="pwc-eyebrow">Accessi permanenti</span> <h2>Le mie Crew</h2> </div> <button class="pwc-btn is-outline" @click="go('crew-create-identity')">+ Crea</button> </header> <button v-for="crew in PITWALL_CONCEPT_CREWS" :key="crew.id" class="pwc-crew-card" :class="`is-${crew.tone}`" @click="go('crew-detail')" > <img class="pwc-crew-image" :src="crewImage(crew.imageId).src" :alt="`Copertina ${crew.name}`" /> <span> <strong>{{ crew.name }}</strong> <small>{{ crew.members }} membri</small> </span> <b aria-hidden="true">›</b> </button> </section> <section class="pwc-recents"> <header> <span class="pwc-eyebrow">Ultimi cinque</span> <h2>Recenti</h2> </header> <article v-for="(person, index) in PITWALL_CONCEPT_RECENTS" :key="person.id" class="pwc-recent-person" :class="{ 'starts-requests': index > 0 && PITWALL_CONCEPT_RECENTS[index - 1]?.action === 'assist' && person.action !== 'assist' }" > <span class="pwc-avatar">{{ person.initials }}</span> <strong>{{ person.handle }}</strong> <button v-if="person.action === 'assist'" class="pwc-btn is-primary" @click="go('live')" > Assisti </button> <div v-else-if="recentPending.includes(person.id)" class="pwc-recent-pending"> In attesa </div> <div v-else class="pwc-recent-request"> <button class="pwc-btn is-outline" :aria-expanded="recentRequestMenu === person.id" @click="recentRequestMenu = recentRequestMenu === person.id ? null : person.id" > Richiedi </button> <div v-if="recentRequestMenu === person.id" class="pwc-request-menu"> <button @click="requestRecentAccess(person.id)">Questa gara</button> <button @click="requestRecentAccess(person.id)">Permanente</button> </div> </div> </article> </section> </aside> </div> </template> <template v-else-if=" screen === 'crew-create-identity' || screen === 'crew-create-people' " > <div class="pwc-flow"> <button class="pwc-back" @click="go('home')"> ← Torna al Pit Wall </button> <h1>Crea una Crew</h1> <div class="pwc-steps"> <button :class="{ active: screen === 'crew-create-identity', done: screen === 'crew-create-people', }" > <b>1</b> Identità</button > <i> </i > <button :class="{ active: screen === 'crew-create-people' }"> <b>2</b> Persone </button> </div> <form v-if="screen === 'crew-create-identity'" class="pwc-flow-card" @submit.prevent="go('crew-create-people')" > <h2>Crea la tua Crew</h2> <label>Nome <em>*</em> <input v-model="crewName" required /> </label > <label >Descrizione <small>opzionale</small > <textarea v-model="crewDescription" rows="4"> </textarea> </label> <fieldset class="pwc-image-picker"> <legend>Immagine Crew</legend> <p>Scegli una delle sei copertine disponibili.</p> <div> <button v-for="image in PITWALL_CONCEPT_CREW_IMAGES" :key="image.id" type="button" :class="{ active: crewImageId === image.id }" :aria-pressed="crewImageId === image.id" @click="crewImageId = image.id"> <img :src="image.src" alt="" /> <span>{{ image.label }}</span> </button> </div> </fieldset> <p class="pwc-info"> ⓘ I membri che accetteranno potranno assistersi reciprocamente durante le gare. </p> <footer> <button type="button" class="pwc-btn" @click="go('home')"> Annulla</button > <button class="pwc-btn is-primary">Continua</button> </footer> </form> <div v-else class="pwc-flow-card is-wide"> <section> <h2>Chi vuoi invitare?</h2> <p>Puoi aggiungere altre persone anche dopo.</p> <label class="pwc-search is-small" > <input placeholder="Cerca nome o nickname…" /> </label> <article v-for="person in invitationPeople" :key="person.id" class="pwc-invite-row" > <div> <strong>{{ person.name }}</strong > <small>{{ person.handle }}</small> </div> <button class="pwc-btn" :class="{ 'is-added': invitees.includes(person.id) }" @click="toggleInvitee(person.id)" > {{ invitees.includes(person.id) ? "Aggiunto" : "Aggiungi" }} </button> </article> </section> <aside> <h3>Invitati · {{ invitees.length }}</h3> <article v-for="id in invitees" :key="id"> <strong>{{ PITWALL_CONCEPT_PEOPLE.find((item) => item.id === id)?.name }}</strong > <button @click="toggleInvitee(id)">Rimuovi</button> </article> </aside> <footer> <button class="pwc-btn" @click="go('crew-create-identity')"> Indietro</button > <button class="pwc-btn" @click="go('crew-detail')">Salta</button > <button class="pwc-btn is-primary" @click="go('crew-detail')"> Crea e invita </button> </footer> </div> </div> </template> <template v-else-if="screen === 'crew-detail'"> <div class="pwc-crew-page"> <button class="pwc-back" @click="go('home')">← Le mie Crew</button> <header> <img class="pwc-crew-page__image" :src="selectedCrewImage.src" :alt="`Copertina ${crewName || 'Crew'}`" /> <div> <h1>{{ crewName || "Apex One Racing" }}</h1> <p>{{ crewDescription }}</p> </div> <div> <span>4 membri</span > <button class="pwc-btn is-outline">+ Invita</button > <button class="pwc-btn">Impostazioni</button> </div> </header> <div class="pwc-crew-grid"> <main> <h2>Attività della Crew</h2> <article class="pwc-race-card"> <header> <span class="pwc-tag is-green">Live</span> <h3>Ferrari 296 GT3 · Monza</h3> <small>Endurance</small> </header> <div class="pwc-race-stats"> <div> <b>P7</b> <small>Posizione</small> </div> <div> <b>18 / 42</b> <small>Giro</small> </div> <div> <b>31 min</b> <small>Stint attuale</small> </div> <div> <b>24–27</b> <small>Pit window</small> </div> </div> <div class="pwc-race-people"> <p> <span>◉</span> <b>Mario Rossi</b> <small>Al volante</small > <button class="pwc-btn" @click="go('live')">Assisti</button> </p> <p> <span>○</span> <b>Luca Bianchi</b> <small>Disponibile</small> </p> <p> <span>◉</span> <b>Enrico Saiani</b > <small>Sta assistendo</small> </p> </div> </article> <h2>Attività recenti</h2> <article class="pwc-recent"> <b>Luca</b> <span>Spa</span> <span>terminata 1 ora fa</span > <button class="pwc-btn">Riepilogo</button> </article> </main> <aside> <h2>Persone · 6</h2> <label class="pwc-search is-small" > <input placeholder="Cerca nella Crew…" /> </label> <div class="pwc-filters"> <button class="active">Tutte</button> <button>Online</button > <button>In attesa</button> </div> <article v-for="(row, index) in [ ['Mario Rossi', 'In gara', 'Al volante'], ['Enrico Saiani', 'Disponibile', 'Proprietario · Tu'], ['Luca Bianchi', 'Disponibile', 'Membro'], ['Sara Neri', 'Offline', 'Membro'], ['Paolo Verdi', 'Invito in attesa', '2 giorni'], ['Andrea Sala', 'Invito in attesa', 'oggi'], ]" :key="row[0]" class="pwc-roster-row" > <span :class="{ online: index < 3, waiting: index > 3 }"> </span > <strong>{{ row[0] }}</strong> <div> <b>{{ row[1] }}</b > <small>{{ row[2] }}</small> </div> <button v-if="index > 3" class="pwc-btn">Annulla</button> </article> </aside> </div> </div> </template> <template v-else> <div class="pwc-live"> <header class="pwc-command"> <button class="pwc-back" @click="go('home')"> ← Assistenza live </button> <div> <h1>Enrico Saiani</h1> <p>Ferrari 296 GT3 · Nürburgring</p> </div> <div class="pwc-command__stats"> <span> <b>P6</b> </span > <span> <small>Giro</small> <b>42/67</b> </span > <span> <small>Stint</small> <b>38:14</b> </span> </div> <div class="pwc-online"> <b>● Live</b> <small>Connessione stabile</small> </div> </header> <section class="pwc-wall"> <div> <small>Al volante</small> <p> <span class="pwc-avatar">ES</span > <b>Enrico Saiani<small>Pilota</small> </b> </p> </div> <div> <small>Al muretto</small> <p> <span class="pwc-avatar">MC</span > <b>Marco<small>Race Engineer</small> </b > <span class="pwc-avatar">LC</span > <b>Luca<small>Strategist</small> </b> </p> </div> <button class="pwc-btn is-outline" @click="guestOpen = true"> + Ospite </button> </section> <div class="pwc-live-grid"> <main class="pwc-race-view"> <header> <h2>{{ liveTab === "timing" ? "Timing" : "Pista" }}</h2> <nav> <button :class="{ active: liveTab === 'timing' }" @click="liveTab = 'timing'" > Timing</button > <button :class="{ active: liveTab === 'track' }" @click="liveTab = 'track'" > Pista </button> </nav> </header> <table v-if="liveTab === 'timing'" class="pwc-timing"> <thead> <tr> <th>Pos</th> <th>Pilota</th> <th>Gap</th> <th>Ultimo giro</th> </tr> </thead> <tbody> <tr> <td>P5</td> <td>A. Costa</td> <td>+2.341</td> <td>1:54.682</td> </tr> <tr class="is-me"> <td>P6</td> <td>Enrico Saiani</td> <td>—</td> <td>1:55.178</td> </tr> <tr> <td>P7</td> <td>M. Bianchi</td> <td>+1.879</td> <td>1:55.924</td> </tr> <tr> <td>P8</td> <td>T. Moretti</td> <td>+3.412</td> <td>1:56.311</td> </tr> </tbody> </table> <div v-else class="pwc-track"> <div class="pwc-track__labels"> <p> <b>P1</b> Leader <small>1:53.217</small> </p> <p class="is-me"> <b>P6</b> Enrico Saiani <small>1:55.178</small> </p> <p> <b>P5</b> Auto davanti <small>+2.341</small> </p> <p> <b>P7</b> Auto dietro <small>+1.879</small> </p> </div> <svg viewBox="0 0 440 230" role="img" aria-label="Mappa Nürburgring" > <path d="M80 55c38-30 95 8 119-10 37-27 96 5 91 42-4 28 60 22 68 58 7 31-34 27-58 40-43 24-56-19-96 4-31 18-83 8-91-27-8-34 42-30 21-62-18-43-86-3-54-45z" /> <circle cx="307" cy="156" r="12" /> <text x="302" y="161">P6</text> </svg> </div> <table class="pwc-sectors"> <thead> <tr> <th>Settori</th> <th>S1</th> <th>S2</th> <th>S3</th> </tr> </thead> <tbody> <tr> <td>Ultimo giro</td> <td class="purple">31.482</td> <td>41.762</td> <td>41.934</td> </tr> <tr> <td>Migliori settori</td> <td>31.214</td> <td>41.533</td> <td>41.621</td> </tr> </tbody> </table> <div class="pwc-metrics"> <div> <small>Passo 5 giri</small> <b>1:55.386</b > <em>−0.287 vs best</em> </div> <div> <small>Carburante</small> <b>22.4 L</b> <em>28%</em> </div> <div> <small>Autonomia</small> <b>10 giri</b> <em>fino a giro 52</em> </div> </div> </main> <aside class="pwc-pit"> <header> <h2>Pit Stop</h2> <p>Strategia da inviare e MFD in macchina</p> </header> <div class="pwc-pit-head"> <b>Campo</b> <b>Strategia</b> <b>MFD live</b> </div> <div class="pwc-pit-row"> <span>Strategia Pit</span > <select v-model="strategy.preset"> <option>Nessun cambio</option> <option>Preset 1</option> </select > <b>N/D</b> </div> <div class="pwc-pit-row"> <span>Aggiungi carburante</span> <div class="pwc-step"> <button @click="strategy.fuel = Math.max(0, strategy.fuel - 1)"> −</button > <b>{{ strategy.fuel }} L</b > <button @click="strategy.fuel++">+</button> </div> <b>0 L</b> </div> <div class="pwc-pit-row"> <span>Cambio gomme</span > <label > <input v-model="strategy.tyres" type="checkbox" /> {{ strategy.tyres ? "Sì" : "No" }}</label > <b>No</b> </div> <div class="pwc-pit-row"> <span>Set pneumatici</span> <div class="pwc-step"> <button @click="strategy.tyreSet = Math.max(1, strategy.tyreSet - 1)" > −</button > <b>{{ strategy.tyreSet }}</b > <button @click="strategy.tyreSet++">+</button> </div> <b>1</b> </div> <div class="pwc-pit-row"> <span>Mescola</span > <select v-model="strategy.compound"> <option>Dry</option> <option>Wet</option> </select > <b>Dry</b> </div> <div class="pwc-pit-row"> <span>Pressioni</span> <div class="pwc-step"> <button @click="stepAll(-1)">−</button> <b>Tutte</b > <button @click="stepAll(1)">+</button> </div> <b>—</b> </div> <div v-for="wheel in ['FL', 'FR', 'RL', 'RR'] as const" :key="wheel" class="pwc-pit-row is-sub" > <span>{{ wheel }}</span> <div class="pwc-step"> <button @click="stepPressure(wheel, -1)">−</button > <b>{{ pressures[wheel].toFixed(1) }}</b > <button @click="stepPressure(wheel, 1)">+</button> </div> <b>25.0</b> </div> <div class="pwc-pit-row"> <span>Sostituisci freni</span > <label > <input v-model="strategy.brakes" type="checkbox" /> {{ strategy.brakes ? "Sì" : "No" }}</label > <b>No</b> </div> <div class="pwc-pit-row"> <span>Prossimo pilota</span > <select v-model="strategy.driver"> <option>Nessun cambio</option> <option>Luca Bianchi</option> </select > <b>Nessun cambio</b> </div> <div class="pwc-pit-row"> <span>Sospensioni</span > <label > <input v-model="strategy.suspension" type="checkbox" /> {{ strategy.suspension ? "Sì" : "No" }}</label > <b>No</b> </div> <div class="pwc-pit-row"> <span>Carrozzeria</span > <label > <input v-model="strategy.bodywork" type="checkbox" /> {{ strategy.bodywork ? "Sì" : "No" }}</label > <b>No</b> </div> <div class="pwc-pit-row"> <span>Tempo richiesto</span> <b>00:30.000</b> <b>—</b> </div> <button class="pwc-send" @click="sent = true"> {{ sent ? "Strategia inviata · mock" : "Invia strategia" }} </button> </aside> </div> <footer class="pwc-stint"> <span>Stint attuale <b>38:14</b> </span > <span>Carburante <b>22.4 L (28%)</b> </span > <span>Fine finestra pit <b>Giri 45–49</b> </span> </footer> </div> </template> <div v-if="guestOpen" class="pwc-modal-backdrop" @click.self="guestOpen = false" > <section class="pwc-guest"> <button class="pwc-close" @click="guestOpen = false">×</button> <h2>Invita un ospite al muretto</h2> <p>L’accesso vale solo per questa gara.</p> <label class="pwc-search is-small" > <input placeholder="Cerca una persona…" /> </label> <article v-for="guest in [ { id: 'alessandro', name: 'Alessandro Neri', initials: 'AN' }, { id: 'giulia', name: 'Giulia Ferrari', initials: 'GF' }, ]" :key="guest.id" > <span class="pwc-avatar">{{ guest.initials }}</span > <strong>{{ guest.name }}</strong > <button class="pwc-btn" @click="guestSelected = guest.id"> {{ guestSelected === guest.id ? "Selezionato" : "Invita" }} </button> </article> <div class="pwc-guest__selected"> <span class="pwc-avatar">{{ guestSelected === "alessandro" ? "AN" : "GF" }}</span > <b >{{ guestSelected === "alessandro" ? "Alessandro Neri" : "Giulia Ferrari" }}<small>Selezionato</small> </b > </div> <button class="pwc-send" @click="guestOpen = false"> Invia invito </button> </section> </div> </section> </template>
