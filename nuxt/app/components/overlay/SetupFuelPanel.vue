...
async function refresh() {
  ...
  } catch { if (token === generation) error.value = 'Anteprima carburante non disponibile.' }
...
function adjust(amount: number) { minutes.value = Math.max(1, Math.min(180, Number(minutes.value) + amount)); mode.value = 'minutes' }
...
<template>
  <section class="fuel-panel">
    ...
    <div v-if="open" class="fuel-body">
      ...
      <div v-if="mode === 'minutes'" class="fuel-duration">
        <button type="button" data-overlay-wheel-action="fuel-minus" aria-label="Un minuto in meno" @click="adjust(-1)">−</button>
        <label><input v-model.number="minutes" type="number" min="1" max="180" step="1" aria-label="Minuti stint" @pointerdown="keyboard" @keydown.stop @blur="api?.trainingOverlayKeyboardEditing?.(false)"> min</label>
      ...
      <template v-if="plan?.totalLitres">
        ...
        <p>{{ (plan.durationMs / 60000).toFixed(1) }} min · {{ plan.consumption }} L/giro · riferimento {{ (plan.referenceLapMs / 1000).toFixed(1) }} s</p>
      ...
      <p v-if="plan && !plan.ok" role="status">{{ plan.reason }}</p>
      <p v-else-if="state && !state.available">{{ state.conflict || 'Fermati ai pit e apri il menu Pausa.' }}</p>
