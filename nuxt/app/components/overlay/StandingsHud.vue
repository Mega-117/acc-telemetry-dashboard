  ...
  >
    ...
    <header class="standings-header">
      ...
      <span class="standings-header__right">
        <strong v-if="model.header.temperatures" aria-label="Temperatura aria e pista">{{ model.header.temperatures }}</strong>
    ...
    <div class="standings-rows">
      ...
      >
        ...
        <strong class="standings-row__pit" :aria-label="row.inPitLane ? 'In pit lane' : 'In pista'">{{ row.inPitLane ? 'P' : '' }}</strong>
        <strong
...
<style scoped>
.standings-hud { position:relative;box-sizing:border-box;display:grid;grid-template-rows:40px minmax(0,1fr);width:900px;height:600px;padding:10px 0;overflow:hidden;border-radius:8px;color:white;background:transparent;font-family:Arial,Helvetica,sans-serif;font-size:18px;font-weight:700;font-variant-numeric:tabular-nums; }
.standings-hud > :not(.hud-overlay-background) { position:relative;z-index:1; }
.standings-header { display:flex;align-items:center;gap:28px;margin:0 10px;overflow:hidden;color:white;font-size:18px;font-weight:700;white-space:nowrap; }
.standings-header__left,.standings-header__right { display:flex;gap:28px;align-items:center; }
...
.standings-row { position:relative;display:flex;gap:8px;align-items:center;box-sizing:border-box;height:24px;min-height:24px;margin:0 10px;overflow:hidden;color:white;font-size:18px;font-weight:700;line-height:1; }
.standings-row.has-progress { height:28px;min-height:28px;padding-bottom:4px; }
.standings-row > strong { position:relative;z-index:1;box-sizing:border-box;overflow:hidden;text-overflow:ellipsis;white-space:nowrap; }
.standings-row__position { display:flex;flex:0 0 30px;align-items:center;justify-content:center;height:24px;color:black;background:yellow; }
...
.standings-row__best,.standings-row__last { flex:0 0 76px;text-align:right; }
...
.standings-row__progress { position:absolute;bottom:0;left:0;z-index:0;height:4px;max-width:100%;background:white;opacity:0.6; }
</style>
