...
export async function loadOverviewSessionSummary(uid: string, sessionId: string): Promise<SessionSummary | null> {
  ...
  if (cached && checkFirebaseCacheFreshness('projection.overviewSummary', cached.cachedAt, 60_000)) return cached.value
  const generation = cacheGeneration
  const snap = await trackedGetDoc(doc(db, `users/${uid}/sessions/${sessionId}`), CALLER)
  return setCache(sessionSummaryCache, key, snap.exists() ? snap.data()?.summary || null : null, generation)
}
