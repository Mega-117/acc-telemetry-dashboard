...
import { auth, signInWithEmailAndPassword, createUserWithEmailAndPassword, sendPasswordResetEmail } from './firebase-auth.js';
class SplashScreen {
    constructor() {
...
class ACCDashboard {
    constructor() {
        ...
        // Driver History Layout Variants
        this.historyStyle = localStorage.getItem('acc_history_style') || 'A'; // A, B, C
        this.historyFocus = localStorage.getItem('acc_history_focus') || '1'; // 1, 2, 3
        this.historySelectedTrack = null; // For filtering
    ...
    setUIState(state) {
        ...
        switch (state) {
            case 'auth':
                // Reset animation classes that might be blocking visibility
                overlay?.classList.remove('hidden', 'exiting', 'morphing');
                authPanel?.classList.remove('hidden');
                console.log('🔐 UI State: AUTH (login/register visible)');
                break;

            case 'loading':
                // Reset animation classes and show loading spinner
                overlay?.classList.remove('hidden', 'exiting', 'morphing');
    ...
    applyTheme(themeName) {
        ...
        // Re-render all charts to pick up new theme colors
    ...
    async handleJSONFirestoreUpload(event) {
        ...
        if (!this.currentUser) {
            console.warn('⚠️ Utente non loggato. Fai login prima di caricare file.');
        ...
        console.log(`📤 Inizio upload Firestore di ${files.length} file...`);
        ...
        for (const file of files) {
            ...
            // 2. If upload successful, also parse and add to local sessions for immediate display
            if (result.status === 'ok') {
                try {
                    const text = await file.text();
                    const data = JSON.parse(text);

                    // Validate and add to local sessions
                    if (this.validateSessionData(data)) {
                        // Check for duplicates in local array
                        const exists = this.sessions.some(s =>
                            s.session_info?.date_start === data.session_info?.date_start &&
                            s.session_info?.track === data.session_info?.track
                        );
                        if (!exists) {
                            this.sessions.push(data);
                            newSessionsAdded++;
                            console.log(`✅ Caricato e aggiunto: ${result.fileName} (ID: ${result.sessionId})`);
                        } else {
                            console.log(`✅ Caricato (già presente localmente): ${result.fileName}`);
                        }
                    } else {
                        console.warn(`⚠️ File non valido per dashboard: ${file.name}`);
                    }
                } catch (parseError) {
                    console.error(`❌ Errore parsing locale: ${file.name}`, parseError);
                }
            } else if (result.status === 'duplicate') {
                console.log(`⚠️ Duplicato: ${result.fileName}`);
            } else {
    ...
    async bootstrapApp(user) {
        console.log('🚀 Bootstrapping app for user:', user.email);
        ...
        try {
            ...
            const firestoreSessions = await fetchSessions(user.uid);
            console.log(`📥 Loaded ${firestoreSessions.length} sessions from Firestore`);

            // 2. TODO: Convert Firestore format to dashboard format if needed
            // For now, just show we have the data
            if (firestoreSessions.length > 0) {
                console.log('✅ Sessions loaded from Firestore, ready to render');
                // this.sessions = firestoreSessions; // Map this to your dashboard format
            } else {
                console.log('ℹ️ No sessions found in Firestore, dashboard will be empty');
            }

            // 3. Enable upload listener
            ...
            // On error, return to auth state (logout the user)
            this.currentUser = null;
            this.sessions = [];
    ...
    initUploadListener() {
        // Only bind once
        if (this.uploadListenerBound) return;
        ...
        if (fileInput) {
            ...
            this.uploadListenerBound = true;
            console.log('📤 Upload listener initialized');
        }
    }
    ...
    initAuthForms() {
        ...
        document.getElementById('authRegisterBtn')?.addEventListener('click', async () => {
            ...
            try {
                ...
                // Set flag so bootstrapApp knows to show success message
        ...
        document.getElementById('authResetBtn')?.addEventListener('click', async () => {
            ...
            const email = prompt('Inserisci la tua email per il reset password:');
            if (!email) return;

            try {
                await sendPasswordResetEmail(auth, email);
                alert('Email di reset inviata! Controlla la tua casella.');
            } catch (error) {
