...
import { handleFileUpload as firestoreUpload, handleMultipleFileUploads } from './upload-sessions.js';
import { fetchAllSessionsRaw } from './sessions-query.js';
// Import modular components
import { themeManager } from './modules/theme-manager.js';
// Import Electron sync module (only active in Electron app)
import { initElectronSync, isElectron, syncAllFiles } from './electron-sync.js';
...
class ACCDashboard {
    ...
    setUIState(state) {
        ...
        switch (state) {
            case 'auth':
                // ===== SHOW OVERLAY (Login OR Upload based on config) =====
                ...
                // 5. Show correct panel based on AUTH_ENABLED
                ...
                // 9. Handle user info visibility
                if (userInfo) {
                    userInfo.style.display = APP_CONFIG.AUTH_ENABLED ? '' : 'none';
                }
            ...
            case 'app':
                ...
                // Handle user info visibility based on auth mode
                if (userInfo) {
                    userInfo.style.display = APP_CONFIG.AUTH_ENABLED ? '' : 'none';
                }
    ...
    clearDashboardUI() {
        ...
        // 5c. Reset sliding indicator to first position (CRITICAL FIX)
        const navbar = document.querySelector('.nav-tabs');
        if (navbar) {
            // Remove all position classes and set to tab-1 (overview)
            navbar.classList.remove('tab-1', 'tab-2', 'tab-3');
            navbar.classList.add('tab-1');
            // Also exit driver history/morphing modes
    ...
    renderSessionStackedBar(sessions) {
        ...
        stackedBar.innerHTML = segments.map(s =>
            `<div class="stacked-segment ${s.type}" style="width:${s.width}%" title="${s.name} â€” ${s.mins} min"></div>`
        ).join('');
    ...
    renderStintsView() {
        ...
        this.setTextContent('stintAirTemp', Math.round(info.start_air_temp || 22) + 'Â°C');
        this.setTextContent('stintRoadTemp', Math.round(info.start_road_temp || 28) + 'Â°C');

        // 6ï¸âƒ£ Statistics Cards
        ...
        // 7ï¸âƒ£ ECG Chart
        // 7ï¸âƒ£ Initialize Advanced Analysis
    ...
    renderStintLapTable(session, bestLapTime) {
        ...
        session.stints.forEach((stint, stintIdx) => {
            ...
            stint.laps.forEach(lap => {
                ...
                // Air temp display
                const airTemp = lap.air_temp ? `${Math.round(lap.air_temp)}Â°` : '-';
            ...
            if (index < stints.length - 1) {
                html += `
                    <div class="sd-pit-divider">
                        <span class="sd-pit-icon">ðŸ”§</span>
    ...
    updateDynamicKPIs(mainLaps, compareLaps) {
        ...
        this.setTextContent('kpiAirTemp', avgAirTemp ? `${avgAirTemp}Â°C` : '-');
        this.setTextContent('kpiRoadTemp', avgRoadTemp ? `${avgRoadTemp}Â°C` : '-');
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            ...
                            afterBody: (items) => {
                                ...
                                if (!lap) return '';
                                const lines = [];
                                if (lap.air_temp) lines.push(`ðŸŒ¡ï¸ Aria: ${Math.round(lap.air_temp)}Â°C`);
                                if (lap.road_temp) lines.push(`ðŸ›£ï¸ Asfalto: ${Math.round(lap.road_temp)}Â°C`);
                                if (!lap.is_valid) lines.push('âš ï¸ CUT');
                                return lines;
                            }
                        }
                    }
    ...
    generateStintSummary(stint, validLaps, invalidLaps, avgLapTime, sectorStats) {
        ...
        if (lengthDesc === 'lungo' && consistency === 'costante') {
            ...
            parts.push(`ðŸŽï¸ Stint ${lengthDesc}, ritmo ${consistency}`);
        }
    ...
    renderDriverHistoryView() {
        ...
        const periodStr = minDate && maxDate ?
            `${minDate.toLocaleDateString('it-IT', { month: 'short', year: 'numeric' })} â€“ ${maxDate.toLocaleDateString('it-IT', { month: 'short', year: 'numeric' })}` : '--';
    ...
    renderCardsLayout(grid, trackData, globalBestTrack) {
        grid.innerHTML = trackData.map(track => {
            ...
            const tooltipText = `${track.sessions.length} sess â€¢ ${this.formatDuration(track.totalTimeMs)}`;
            ...
            <div class="dh-track-card${isBest ? ' is-personal-best' : ''}" 
                ...
                <div class="dh-track-bests">
                    <div class="dh-track-best-item dh-best-race${hasRace ? '' : ' no-data'}">
                        <span class="dh-best-icon">ðŸ</span>
                    ...
                    <div class="dh-track-best-item dh-best-quali${hasQuali ? '' : ' no-data'}">
                        <span class="dh-best-icon">â±ï¸</span>
    ...
    renderTableLayout(grid, trackData, globalBestTrack) {
        ...
        const sortIcon = (key) => {
            if (this.tableSortKey === key) {
                return `<span class="sort-icon">${this.tableSortDir === 'asc' ? 'â–²' : 'â–¼'}</span>`;
            }
        ...
        grid.innerHTML = `
            <table class="dh-tracks-table">
                <thead>
                    <tr>
                        ...
                        <th class="${this.tableSortKey === 'race' ? 'sorted' : ''}" onclick="dashboard.sortTable('race')">ðŸ Race ${sortIcon('race')}</th>
                        <th class="${this.tableSortKey === 'quali' ? 'sorted' : ''}" onclick="dashboard.sortTable('quali')">â±ï¸ Quali ${sortIcon('quali')}</th>
            ...
            return `
                        <tr onclick="dashboard.openTrackDetail('${track.name}')">
                            <td class="track-name">${track.name === globalBestTrack ? 'ðŸ‘‘ ' : ''}${this.getTrackDisplayName(track.name)}</td>
    ...
    openTrackDetail(trackName) {
        ...
        if (raceRec.time < Infinity) {
            ...
            this.setTextContent('dhRaceDate', raceRec.date ? `ðŸ“… ${new Date(raceRec.date).toLocaleDateString('it-IT')}` : '--');
            this.setTextContent('dhRaceWeather', raceRec.weather ? `â˜ï¸ ${raceRec.weather}` : '--');
            this.setTextContent('dhRaceTemp', raceRec.temp ? `ðŸŒ¡ï¸ ${raceRec.temp}Â°C` : '');
            this.setTextContent('dhRaceGrip', raceRec.grip ? `ðŸ›ž ${raceRec.grip}` : '');
        ...
        if (qualiRec.time < Infinity) {
            ...
            this.setTextContent('dhQualiDate', qualiRec.date ? `ðŸ“… ${new Date(qualiRec.date).toLocaleDateString('it-IT')}` : '--');
            this.setTextContent('dhQualiWeather', qualiRec.weather ? `â˜ï¸ ${qualiRec.weather}` : '--');
            this.setTextContent('dhQualiTemp', qualiRec.temp ? `ðŸŒ¡ï¸ ${qualiRec.temp}Â°C` : '');
            this.setTextContent('dhQualiGrip', qualiRec.grip ? `ðŸ›ž ${qualiRec.grip}` : '');
                    ...
                    return `
                        <div class="dh-session-item" onclick="dashboard.openSessionFromHistory(${index})">
                            <div>
                                ...
                                <div class="dh-session-info">${sessionType} â€¢ ${weather}</div>
                            </div>
    ...
    getTrackDisplayName(trackId) {
        const trackNames = {
            ...
            'nurburgring': 'NÃ¼rburgring GP',
            'nurburgring_24h': 'NÃ¼rburgring 24h',
    ...
    async bootstrapApp(user) {
        ...
        try {
            ...
            if (this.registrationJustCompleted) {
                this.registrationJustCompleted = false;
                setTimeout(() => {
                    alert('âœ… Registrazione completata! Benvenuto, ' + user.email.split('@')[0] + '!');
                }, 300);
            }
    ...
    initAuthForms() {
        ...
        const mapFirebaseAuthError = (errorCode) => {
            const errorMap = {
                ...
                'auth/user-disabled': 'Questo account Ã¨ stato disabilitato.',
        ...
        ['authRegisterNickname', 'authRegisterEmail', 'authRegisterPassword', 'authRegisterConfirm'].forEach(id => {
        ...
        document.getElementById('authLoginBtn')?.addEventListener('click', async () => {
            ...
            try {
                ...
                const { signInWithEmailAndPassword, signOut, sendEmailVerification } = await import("https://www.gstatic.com/firebasejs/12.7.0/firebase-auth.js");

                const userCredential = await signInWithEmailAndPassword(auth, email, password);

                // Check email verification based on config
                if (!userCredential.user.emailVerified) {
                    if (APP_CONFIG.REQUIRE_EMAIL_VERIFICATION) {
                        // Block access - logout and show error
                        await signOut(auth);
                        showError('login', 'Devi verificare la tua email prima di accedere. Controlla la tua casella di posta.');
                        setLoading('authLoginBtn', false);
                        return;
                    } else {
                        // Just warn (non-blocking)
                        console.log('[AUTH] Warning: Email not verified, but access allowed');
                    }
                }
        ...
        document.getElementById('authRegisterBtn')?.addEventListener('click', async () => {
            ...
            // Client-side validation
            if (!nickname) {
                markInputError('authRegisterNickname');
                showError('register', 'Scegli un nickname.');
                return;
            }
            if (nickname.length < 3) {
                markInputError('authRegisterNickname');
                showError('register', 'Il nickname deve essere di almeno 3 caratteri.');
                return;
            }
            if (!email) {
            ...
            try {
                ...
                const { createUserWithEmailAndPassword, updateProfile, sendEmailVerification } = await import("https://www.gstatic.com/firebasejs/12.7.0/firebase-auth.js");

                // 1. Create user
                const userCredential = await createUserWithEmailAndPassword(auth, email, password);

                // 2. Set display name (nickname)
                ...
                console.log('[AUTH] Registration complete, verification email sent to:', email);
        ...
        document.getElementById('authResetBtn')?.addEventListener('click', () => {
            // Hide login panel, show reset panel with animation
            ...
            if (authTabs) authTabs.style.display = 'none'; // Hide tabs during reset
        ...
        document.getElementById('authBackToLoginBtn')?.addEventListener('click', () => {
            ...
            if (authTabs) authTabs.style.display = ''; // Show tabs again
        ...
        document.getElementById('authResetSubmitBtn')?.addEventListener('click', async () => {
            clearError('reset');

            const email = document.getElementById('authResetEmail').value.trim();

            if (!email) {
                showError('reset', 'Inserisci la tua email.');
                markInputError('authResetEmail');
                return;
            }

            setLoading('authResetSubmitBtn', true, 'Invio...');
            ...
            try {
                ...
                // Show success briefly, then animate back to login
                const resetPanel = document.getElementById('resetPanel');
                if (resetPanel) {
                    resetPanel.innerHTML = `
                        <div class="reset-success">
                            <span class="reset-success-icon">✅</span>
                            <h3 class="reset-title">Email Inviata!</h3>
                            <p class="reset-description">Controlla la tua casella email per il link di reset.</p>
                        </div>
                    `;
                }

                // After 2.5s, return to login
                setTimeout(() => {
                    // Restore reset panel HTML
                    if (resetPanel) {
                        resetPanel.innerHTML = `
                            <h3 class="reset-title">Recupera Password</h3>
                            <p class="reset-description">
                                Inserisci la tua email. Ti invieremo un link per reimpostare la password.
                                <br><small>Controlla anche la cartella spam.</small>
                            </p>
                            <input type="email" id="authResetEmail" placeholder="Email" class="form-input" autocomplete="email">
                            <div id="resetError" class="form-error"></div>
                            <button id="authResetSubmitBtn" class="btn btn-primary">INVIA LINK</button>
                            <button id="authBackToLoginBtn" class="btn-link">← Torna al login</button>
                        `;
                        resetPanel.classList.remove('active');

                        // Re-bind event listeners for restored elements
                        this.bindResetPanelEvents();
                    }
                ...
                const userMessage = mapFirebaseAuthError(error.code);
                showError('reset', userMessage);
                setLoading('authResetSubmitBtn', false);
            }
        });

    }

    // Helper to re-bind reset panel events after DOM restoration
    bindResetPanelEvents() {
        // Back to login
        document.getElementById('authBackToLoginBtn')?.addEventListener('click', () => {
            const loginPanel = document.getElementById('loginPanel');
            const resetPanel = document.getElementById('resetPanel');
        ...
        // Submit reset (recursive call to main handler logic)
        document.getElementById('authResetSubmitBtn')?.addEventListener('click', async () => {
            // Trigger the same logic - simplified: just show alert for now
            const email = document.getElementById('authResetEmail')?.value.trim();
            if (!email) {
            ...
            try {
                const { sendPasswordResetEmail } = await import("https://www.gstatic.com/firebasejs/12.7.0/firebase-auth.js");
                const module = await import('./firebase-auth.js');
                await sendPasswordResetEmail(module.auth, email);
                alert('✅ Email di reset inviata!');
                document.getElementById('authBackToLoginBtn')?.click();
            } catch (e) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize Electron sync (only works in Electron app)
    try {
        initElectronSync();
    } catch (e) {
        console.log('[DASHBOARD] Electron sync not available:', e.message);
    }

    // Initialize custom titlebar (Electron only)
    if (typeof window.electronAPI !== 'undefined' && window.electronAPI.isElectron) {
        ...
        if (titlebar) {
            ...
            // First button (circular arrow) = Refresh page
            document.getElementById('titlebarSyncBtn')?.addEventListener('click', () => {
                window.electronAPI.pageRefresh();
            });

            // Second button (double arrows) = Sync files
            document.getElementById('titlebarRefreshBtn')?.addEventListener('click', async () => {
                if (window.electronSync?.sync) {
                    const result = await window.electronSync.sync();
                    console.log('[TITLEBAR] Sync result:', result);
                }
            });

            // Minimize
            document.getElementById('titlebarMinBtn')?.addEventListener('click', () => {
                window.electronAPI.windowMinimize();
            });

            // Maximize/Restore
            document.getElementById('titlebarMaxBtn')?.addEventListener('click', () => {
                window.electronAPI.windowMaximize();
            });

            // Close
            document.getElementById('titlebarCloseBtn')?.addEventListener('click', () => {
                window.electronAPI.windowClose();
            });

            console.log('[DASHBOARD] Custom titlebar initialized');
        }
    }
});
