...
export class ThemeManager {
     ...
     */
    getChartColors() {
        // Chart accent: prioritize RME-specific --chart-accent, fallback to --theme-accent
        const chartAccent = this.getCssVar('--chart-accent', '');
        const chartAccentRgb = this.getCssVar('--chart-accent-rgb', '');

        return {
            // Theme accents - use chart-specific if available
            accent: chartAccent || this.getCssVar('--theme-accent', '#6366f1'),
            accentRgb: chartAccentRgb || this.getCssVar('--theme-accent-rgb', '99, 102, 241'),
