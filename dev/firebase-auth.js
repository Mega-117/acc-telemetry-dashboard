...
if (APP_CONFIG.AUTH_ENABLED) {
    ...
    onAuthStateChanged(auth, async (user) => {
        ...
        if (user) {
            ...
            // Save user identity for Electron (file ownership)
            if (typeof window.electronAPI !== 'undefined' && window.electronAPI.saveUserIdentity) {
                try {
                    await window.electronAPI.saveUserIdentity({
                        userId: user.uid,
                        email: user.email,
                        displayName: user.displayName || user.email.split('@')[0]
                    });
                    console.log('[AUTH] User identity saved locally');
                } catch (e) {
                    console.warn('[AUTH] Could not save user identity:', e);
                }
            }
            ...
            // Clear user identity on logout (Electron only)
            if (typeof window.electronAPI !== 'undefined' && window.electronAPI.clearUserIdentity) {
                try {
                    await window.electronAPI.clearUserIdentity();
                    console.log('[AUTH] User identity cleared');
                } catch (e) {
                    console.warn('[AUTH] Could not clear user identity:', e);
                }
            }
        }
