 ...
 * @param {Object} options - Options { skipOwnerCheck: boolean }
 */
export async function uploadOrUpdateSession(rawObj, rawText, fileName, uid, options = {}) {
    try {
        // ===== OWNER VALIDATION =====
        const fileOwnerId = rawObj.ownerId || null;

        // Se il file ha un ownerId diverso dall'utente corrente, salta
        if (fileOwnerId && fileOwnerId !== uid && !options.skipOwnerCheck) {
            console.log(`[UPLOAD] Skipped ${fileName}: owner mismatch (file: ${fileOwnerId}, current: ${uid})`);
            return {
                status: 'skipped',
                fileName,
        ...
        // Se il file non ha ownerId, avvisa ma procedi (file pre-login)
        if (!fileOwnerId) {
            console.log(`[UPLOAD] Warning: ${fileName} has no ownerId (pre-login session)`);
        }
        // ===== END OWNER VALIDATION =====
