...
class ACCDashboard {
    ...
    updateDynamicKPIs(mainLaps, compareLaps) {
        ...
        if (standardBox && compareBox) {
            standardBox.style.display = isCompare ? 'none' : 'grid';
            compareBox.style.display = isCompare ? 'flex' : 'none';
            compareBox.classList.toggle('hidden', !isCompare);
        }
    ...
    updateEcgChart(mainLaps, compareLaps) {
        ...
        // Colors - Best lap is GREEN, invalid is RED, normal is BLUE (or ORANGE for compare)
        // In compare mode: Dataset A = Blue, Dataset B = Orange, but best laps still GREEN
        const sessionBest = this.currentSession.session_info.session_best_lap;
        const pointColors = validMain.map(l => {
            // Always show best lap as green, even in compare mode
            if (l.is_valid && l.lap_time_ms === sessionBest) return '#22c55e'; // GREEN for best
            if (!l.is_valid) return '#ef4444'; // Red for invalid
        ...
        datasets.push({
            ...
            pointBorderColor: pointColors, // Match border color to fix purple border on green best lap
            pointBorderWidth: 2,
            segment: {
                borderColor: ctx => {
                    if (state.compareModeEnabled) return '#6366f1';
                    const lap = validMain[ctx.p0DataIndex];
                    if (!lap?.is_valid) return '#ef4444';
                    return '#6366f1';
                }
            }
        });
        ...
        if (state.trend && dataMain.length > 1) {
            ...
            datasets.push({
                label: 'Trend',
                data: trendData,
                borderColor: '#f59e0b', // Yellow dashed
                borderDash: [6, 4],
                borderWidth: 2,
                pointRadius: 0,
                tension: 0,
            ...
            if (state.compareModeEnabled && compareLaps.length > 0) {
                ...
                if (dataCompare.length > 1) {
                    ...
                    datasets.push({
                        ...
                        borderColor: '#facc15', // Lighter yellow for distinction
                        borderDash: [6, 4],
                        borderWidth: 2,
                        pointRadius: 0,
                        tension: 0,
    ...
    hideDriverHistory() {
        ...
        document.querySelectorAll('.nav-tab').forEach(t => t.classList.remove('active'));
        document.querySelector('.nav-tab[data-view="overview"]')?.classList.add('active');
    }
        ...
        this.charts.historyChart = new Chart(ctx, {
            ...
            options: {
                ...
                scales: {
                    ...
                    y: {
                        reverse: true, // Lower times at top
                        grid: { color: 'rgba(255,255,255,0.05)' },
                        border: { color: 'rgba(255,255,255,0.1)' },
                        ticks: {
